...
| --- | --- | --- | --- | --- |
| `dev` | push to `develop` | `fullstackmanish-dev` | https://fullstackmanish-dev.web.app | no |
| `uat` | push to `main` | `fullstackmanish-uat` | https://fullstackmanish-uat.web.app | no |
| `prod` | tag `v*`, or manual run | `fullstackmanish` | https://fullstackmanish.web.app | yes |
| preview | pull request | channel on the dev site | ephemeral | no |
...
| Type | Name | Value |
| --- | --- | --- |
| Secret | `FIREBASE_SERVICE_ACCOUNT` | Service account JSON (Firebase Hosting Admin) |
...
Get the service account from *IAM & Admin → Service Accounts → Keys → Add key*,
granting it **Firebase Hosting Admin** and nothing broader. `deploy.yml` writes it
to `$RUNNER_TEMP` and exposes it as `GOOGLE_APPLICATION_CREDENTIALS`, which is the
CI authentication path documented for firebase-tools; the file is deleted again in
an `always()` step.
...
> **Two things changed here.** `main` now deploys to **UAT**, not production, so
> shipping to production is `git tag v1.0.0 && git push --tags` (or a manual run
> choosing `prod`). And the first production deploy **replaces** the older
> portfolio currently served at https://fullstackmanish.web.app — to keep it,
> point `prod.firebaseSite` in `env.config.mjs` at a different Hosting site and
> update `firebase.json` and `.firebaserc` to match.
...
- [x] `develop` branch pushed — triggers the dev deploy
- [x] Pushed to `main` — triggers the UAT deploy
- [x] CI green on `main`: lint, typecheck and all three stage builds
- [ ] Tagged `v1.0.0` — triggers the production deploy
